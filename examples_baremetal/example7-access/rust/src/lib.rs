...
use std::str::from_utf8 as from_utf8;
...
pub fn process_instruction(
	...
	msg!("[lib] Provided access account key: {:?}", access_account.key);
	...
	let mut access_struct = try_from_slice_unchecked::<AccessStruct>(&access_account.data.borrow())?;	
	...
	if instruction_data.len() > 0{		
		...
		if caller.is_signer {
			...
			let (seed_length, rest) = instruction_data.split_first().ok_or(ProgramError::BorshIoError("Invalid parameters passed".to_string()))?;		

			// Get seed from up to the key size as a string				
			let seed = from_utf8(rest.get(..*seed_length as usize).unwrap()).unwrap().to_string();
			...
			if derived_pda == *access_account.key {
				...
				word = from_utf8(rest.get(*seed_length as usize + 1..).unwrap()).unwrap().to_string();
			}			
        }			
